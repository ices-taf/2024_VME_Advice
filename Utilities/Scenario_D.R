...
# add VMEs
VME <- read.csv(paste("1-Input data/VME data repository/VME observations and csquares/VME_csquares_datacall_",
                      datacallyear,"_eu.csv",sep=""),header=T,sep=",",row.names = NULL)
...
#vmsreg <- readRDS(paste("1-Input data/VMS data repository/All_VMS_datacall",datacallyear_VMS,".rds",sep=""))  
vmsreg <- readRDS("1-Input data/VMS data repository/SARdata_2009_2022.rds")
vmsreg <- vmsreg %>% replace(is.na(.), 0)
...
# fill holes
tt <- st_union(st_make_valid(st_as_sf(sce22)))
...
# and clean
rm(list=setdiff(ls(), c("pathdir" , "pathdir_nogit","datacallyear","datacallyear_VMS","refyear","SAR_threshold", "geopack", "geopackORIG", "assyearprev", "PubYearPrevAdvice")))
