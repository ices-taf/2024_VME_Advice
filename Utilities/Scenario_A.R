...
# fill holes
  tt <- st_union(st_make_valid(st_as_sf(sce11)))
...
# add buffer to make sure that filling holes works
  ...
  midlat  <- round(st_coordinates(st_centroid(reg))[,2]) # get midpoint
...
# and clean
  rm(list=setdiff(ls(), c("pathdir" , "pathdir_nogit","datacallyear","datacallyear_VMS", "geopack", "geopackORIG", "assyearprev", "PubYearPrevAdvice")))
