...
# load VME observations
  VMEobs <- read.csv(paste(pathdir,paste(
  ...
  # get all presences in the EU list
  ...
  VMEobs <- subset(VMEobs,(VMEobs$VME_Indicator %in% VMEindic | VMEobs$HabitatType %in% VMEhabs)) 
...
## get 0.25 c-square grid to combine all data
  ...
  for (iGrid in 1:5){
    ...
    sce12 <- subset(quar_grid,quar_grid@data$summing > 0)
    sce12 <- spTransform(sce12, CRS("+init=epsg:4326"))
    assign(nam[iGrid],sce12)  
  }
   
# save 0.25 c-sq output
  sce12 <- rbind(north1,north2,north3,north4,south)
  sce12 <- sce12[,-1]
  rownames(sce12) <- NULL
  sce12 <- sce12[!(duplicated(sce12@data$uni)),]
  save(sce12,file=paste(pathdir,paste("2-Data processing/VME_polygons",datacallyear,sep="_"),
                        "sce12_quarter_csq_grid.RData",sep="/"))
  
  # fill holes
  tt <- st_union(st_make_valid(st_as_sf(sce12)))
  
  # add buffer to make sure that filling holes works
  reg <- st_transform(tt, 3035) 
  reg <- st_buffer(reg, dist=0)
  reg <- st_transform(reg, 4326)
  reg <- st_sf(st_cast(reg,"POLYGON"))
  ...
  write_sf(clos12, paste0(paste(pathdir,paste("2-Data processing/VME_polygons",datacallyear,sep="_"),sep="/"),
                          "/Scenario1_option2.shp"))
  
  # and clean
  #rm(list=setdiff(ls(), c("pathdir" ,"datacallyear","datacallyear_VMS","geopack", "geopackORIG", "assyearprev", "PubYearPrevAdvice")))
